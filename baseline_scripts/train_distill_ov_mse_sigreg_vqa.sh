...
torchrun --standalone \
    ...
    --percent_data 1.0 \
    --output_dir "training/llava_ov-0.5B_mse_sigreg_vqa" \
...
python eval_mmeb_2.py \
  --model_name "training/llava_ov-0.5B_mse_sigreg_vqa/checkpoint-epoch-0" \
  --encode_output_path "./MMEB-eval_outputs/llava_ov-0.5B_mse_sigreg_vqa" \
  --lora True \
  --lora_r 64 \
  --lora_alpha 64 \
  --pooling eos \
