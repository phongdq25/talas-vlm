...
torchrun --standalone \
    ...
    --percent_data 1.0 \
    --output_dir "training/FastVLM-0.5B_emo_sigreg_cls" \
...
python eval_mmeb_2.py \
  --model_name "training/FastVLM-0.5B_emo_sigreg_cls/checkpoint-epoch-0" \
  --encode_output_path "./MMEB-eval_outputs/FastVLM-0.5B_emo_sigreg_cls" \
  --lora True \
  --lora_r 64 \
  --lora_alpha 64 \
  --pooling eos \
