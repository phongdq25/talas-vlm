...
from src.model.processor import LLAVA_NEXT, QWEN2_VL, QWEN2_5_VL, PHI3V, QWEN2_VL_TOKENSELECTION, QWEN2_5_VL_TOKENSELECTION, process_vlm_inputs_fns, INTERN_VL3, LLAVA_QWEN2, LLAVA_QWEN2_OLD
...
class EvalCollator:
    ...
    def __call__(self, examples):
        ...
        elif(self.model_args.model_backbone==LLAVA_QWEN2 or self.model_args.model_backbone==LLAVA_QWEN2_OLD):
            inputs = process_vlm_inputs_fns[self.model_args.model_backbone](examples,
                                            processor = self.processor)
...
class MultimodalEvalDataCollator:
    ...
    def __call__(self, examples):
        ...
        if self.encode_side == 'qry':
            ...
            return processed_qry_inputs, dataset_infos
        else:
            ...
            cand_inputs = self._get_batch_inputs(examples, "cand_text", "cand_image")
            processed_cand_inputs = process_fn(cand_inputs, processor=self.processor, max_length=self.data_args.max_len)
            dataset_infos = [e["dataset_infos"] for e in examples]
            return processed_cand_inputs, dataset_infos
